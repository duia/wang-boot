...
import com.wpc.common.utils.math.RandomUtils;
...
public class FileUtils {
    ...
    private static String getName(int a) {
    	return (a + 1000 + "").substring(1);
	}

	public static void main(String[] args) {
    	String inputString = "C:\\Users\\admin\\Downloads\\火影忍者漫画全集\\370~500 话";
    	int i = 0;
		try {
			File outputFile = new File("E:\\tmp\\copy");
			if (!outputFile.exists()) {		//文件夹
				outputFile.mkdirs();
			}
			...
			int hua = 490;
			for (int a = 0; a < 20; a++) {
				File inputFile = new File(inputString + "\\第"+(hua+a)+"话");
				if (inputFile.exists()) {
					for (File child : inputFile.listFiles()) {
						if (child.isFile()) {        //文件
							++i;
							String fileName = child.getName();
							String newName = outputFile.getPath() + "/" + getName(i) + fileName.substring(fileName.lastIndexOf("."));
							System.out.println(newName);
							copySimpleFile(child, new File(newName), true);
						}
					}
				}
			}
