	...
	private class ExcelSAXHandler extends DefaultHandler {
		...
		public void startElement(String uri, String localName, String qName, Attributes attributes) throws SAXException {
			...
			if ("c".equals(qName)) {
				...
				} else if (StringUtils.hasText(cellStyleStr)) {
			...
			if ("t".equals(qName)) {
				...
				case NUMBER:
					if (HSSFDateUtil.isADateFormat(this.formatIndex, this.formatString)) {
						value = DateFormatUtils.formatDate(HSSFDateUtil.getJavaDate(Double.parseDouble(this.lastContents.toString())), "yyyy-MM-dd");
					} else if (StringUtils.isEmpty(this.formatString)) {
						value = this.lastContents.toString();
					} else {
